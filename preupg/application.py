...
class Application(object):
    ...
    def run_third_party_modules(self, dir_name):
        ...
        for module_set, all_xccdf_xml_path in \
            ...
            # This function prepares the XML and generates an HTML
    ...
    def generate_report(self):
        ...
        self.common.prep_symlinks(
            self.module_set_copy_path,
            self.rename_custom_module_set(self.module_set_dirname))
        self.all_xccdf_xml_copy_path = os.path.join(
            self.module_set_copy_path,
            settings.all_xccdf_xml_filename)
        ret = XccdfHelper.update_platform(self.all_xccdf_xml_copy_path)
        if ret:
    ...
    def scan_system(self):
        ...
        try:
            ...
            log_message("The module {0} does not exist."
    ...
    def determine_module_set_location_info(self):
    ...
    def run(self):
        ...
        if self.conf.scan and self.conf.contents:
            log_message("Use either --scan or --contents option, not both.")
            return ReturnValues.INVALID_CLI_OPTION

        return_code = self.determine_module_set_location_info()
        if return_code:
            return return_code
